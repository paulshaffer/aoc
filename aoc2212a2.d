...
auto progStartTime = MonoTime.currTime;
	...
	// write moves counted and ending node
	queue[0].writeln;
	grid[queue[0].row][queue[0].col].writeln;

	// Extra investigation to print an arrow path of the route
	//	taken through the graph
	// Walk backwards through the 'grid' using the 'previous node' data and
	//	add path arrows to 'grid' structs
