...
auto datastream = file.readln;
enum mark = 4;
int markPos;

auto progStartTime = MonoTime.currTime;

for(int i=0; i <= (datastream.length - mark); ++i) {
	auto sl = datastream[i .. (i + mark)];
	//write(i);

	if (sl[0]!=sl[1] && sl[0]!=sl[2] && sl[0]!=sl[3] && 
		sl[1]!=sl[2] && sl[1]!=sl[3] && sl[2]!=sl[3]) {
